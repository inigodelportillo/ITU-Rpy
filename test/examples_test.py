...
def suite():
    ...
    suite.addTest(TestSingleLocationVsUnavailability("test_single_location_vs_p"))
    ...
    def test_map_africa():
        ...
        try:
            m = itur.plotting.plot_in_map(
                Att.value, lat, lon, cbar_text="Atmospheric attenuation [dB]", cmap="magma"
            )
            ...
            m = itur.plotting.plot_in_map(
                T.value, lat, lon, cbar_text="Surface mean temperature [C]", cmap="RdBu_r"
            )
...
class TestMultipleLocations(test.TestCase):
    ...
    def test_multiple_locations():
        ...
        ax.bar(
            city_idx + 1.5 * width, As.value, width, label="Scintillation attenuation"
        )
...
class TestSingleLocation(test.TestCase):
    ...
    def test_single_location():
        ...
        itur.models.itu618.rain_attenuation_probability(lat, lon, el, hs)
        ...
        itur.models.itu676.zenit_water_vapour_attenuation(lat, lon, p, f, h=hs)
...
class TestGaseousAttenuation(test.TestCase):
    ...
    def test_gaseous_attenuation():
        ...
        plt.grid(which="major", linestyle=":", color="black")
        plt.title(
            "FIGURE 2. - Specific attenuation in the range 50-70 GHz"
