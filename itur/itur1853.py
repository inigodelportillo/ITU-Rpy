...
from itur618 import fit_rain_attenuation_to_lognormal, rain_percentage_probability
from itur840 import lognormal_approximation_coefficient, specific_attenuation_coefficients
from itur836 import total_water_vapour_content
from itur676 import zenit_water_vapour_attenuation
from itur1510 import surface_mean_temperature
from utils import load_data, dataset_dir, prepare_input_array, prepare_output_array
...
class _ITU1853_1():
    ...
    def rain_attenuation_synthesis(self, lat, lon, f, el, hs, T, Ts=1, n=None):
        ...
        if n is None:
            n = np.random.rand((T + 2e5)//Ts)
        # D2, D3 : Filter the noise time series with a recursive low-pass filter
        rho = np.exp(-beta*Ts)
        X = lfilter([np.sqrt(1-rho**2), rho], [1], n , 0)
        # D4: Compute Y_rain
    ...
    def scintillation_synthesis(self, T, f_c = 0.1, Ts=1, f_c=0.01):
        """
        For Earth-space paths, the time series synthesis method is valid for 
        frequencies between 4 GHz and 55 GHz and elevation angles between
        5 deg and 90 deg. 
        ...
        H_f = np.where(freqs <= f_c, 1, 10**((np.log10(freqs) - np.log10(f_c))*(-8/3)))
        H_f = H_f[0:int(T + 2e5)]
        sci = fftnoise(np.fft.fftshift(H_f))
        return A_rain        
    ...
    def cloud_liquid_water_synthesis(self, lat, lon, T, Ts=1, n=None):
        ...
        X_1 = lfilter([np.sqrt(1-rho_1**2), rho_1], [1], n , 0)
        rho_2 = np.exp(-beta_2*Ts)
        X_2 = lfilter([np.sqrt(1-rho_2**2), rho_2], [1], n , 0)
    ...
    def integrated_water_vapour_synthesis(self, lat, lon, T, Ts=1, n=None):
        ...
        Vi = np.array([total_water_vapour_content(lat, lon, p_i) for p_i in ps])
        ...
        G_v = lfilter([np.sqrt(1-rho**2), rho], [1], n , 0)
    ...
    def total_attenuation_synthesis(self, lat, lon, f, el, p, hs, T, Ts=1):
        ...
        def a_Fade(p):
            return -0.061 * np.log10(p)**3 + 0.072 * np.log10(p)**2 - 1.71*np.log10(p) + 3
        def a_Enhanc(p):
            return -0.0597 * np.log10(p)**3 - 0.0835 * np.log10(p)**2 - 1.258*np.log10(p) + 2.672
        ...
        Ar = self.rain_attenuation_synthesis(lat, lon, f, el, hs, T + 5e6, n=n_R)
        Ar = Ar[5e6:]
        
        # Step C7: Compute the cloud integrated liquid water content time series 
