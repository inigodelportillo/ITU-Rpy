...
def compute_distance_earth_to_earth( lat_p, lon_p, lat_grid, lon_grid ):
    ...
    -------
        d : numpy array
        Distance between the point P and each point in (lat_grid, lon_grid) (km)
...
def regular_lat_lon_grid( resolution_lat = 1 , resolution_lon = 1 , lon_start_0 = False,
    ...
    '''
    Build latitude and longitude coordinate matrix with resolution resolution_lat, 
    resolution_lon
    ...
    if lon_start_0 :
        lon, lat = np.meshgrid( np.arange(lon_min + 180.0, lon_max + 180.0, resolution_lon ), 
...
if __name__=='__main__':
    ...
    folders = ['453/', '836/', '837/' ,'839/', '840/', '1511/']
    for f in folders:
