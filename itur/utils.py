...
def plot_in_map(data, lat=None, lon=None, lat_min=None, lat_max=None,
    ...
    **kwargs: dict
    ...
    -------
    m : Cartopy
        The map object generated by Cartopy
    '''
    ...
    try:
        ...
        raise RuntimeError('Cartopy is not installed and therefore plot_in_map'
                           ' cannot be used')

    #### Ensure that lat and lon was provided
    ...
    ####if given a dataset and not boundaries, make the boundaries from the dataset
    ...
    ####If axes were not provided make basic axes.
    if ax is None:
        fig = plt.figure(figsize=figsize)
        ax = fig.add_subplot(111)
        
        #### Use cartopy to make the plot
    #projection is PlateCarree
    m = plt.axes(projection = ccrs.PlateCarree())

    #### create coastline and country borders features
    m.add_feature(cfeature.COASTLINE, alpha = 0.5)
    m.add_feature(cfeature.BORDERS, alpha = 0.5)

    ####use max and min lat and lon as the boundaries
    m.set_extent([lon_min, lon_max, lat_min, lat_max], ccrs.PlateCarree())

    ####draw the lat and lon gridlines to the map
    grid_lines = m.gridlines(draw_labels = True)
    grid_lines.xformatter = LONGITUDE_FORMATTER
    grid_lines.yformatter = LATITUDE_FORMATTER

    ####Create a heatmap with with the given data and create a reference colorbar. 
    im = m.pcolormesh(lon, lat, data, transform = ccrs.PlateCarree(), **kwargs)
    m.set_title(cbar_text, pad = 25)
    plt.colorbar(im, pad = .1)
