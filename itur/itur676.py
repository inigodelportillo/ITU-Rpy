...
from itur836 import total_water_vapour_content, surface_water_vapour_density
from itur835 import standard_pressure, standard_temperature
from itur453 import radio_refractive_index
from utils import prepare_quantity, prepare_output_array, prepare_input_array, load_data, dataset_dir
...
class __ITU676():
    ...
    * P.676-11 (09/16) (Current version)
    ...
    def gaseous_attenuation_terrestrial_path(self,  r, f, el, rho, P, T, mode):
        ...
        return self.instance.gaseous_attenuation_terrestrial_path(self, r, f, el, rho, P, T, mode)

    def gaseous_attenuation_slant_path(self, f, el, rho, P, T, mode):
        # Abstract method to compute the gaseous attenuation over a slant path
        return self.instance.gaseous_attenuation_slant_path(self, f, el, rho, P, T, mode)
        
    def zenit_water_vapour_attenuation(self, lat, lon, p, f, V_t=None, alt=None):
        # Abstract method to compute the water vapour attenuation over the slant path
        return self.instance.zenit_water_vapour_attenuation(lat, lon, p, f, V_t, alt)
        
    def gammaw_approx(self, f, p, rho, t):
        # Abstract method to compute the specific attenuation due to water vapour
        return self.instance.gammaw_approx(f, p, rho, t)
        
    def gamma0_approx(self, f, p, t):
        # Abstract method to compute the specific attenuation due to dry atmoshere
...
class _ITU676_10():
    ...
    def gammaw_approx(self, f, P, rho, T):
        ...
        gammaw = ( (3.98 * eta1 * np.exp(2.23 * (1 - rt))) / ((f -  22.235)**2 +  9.42 * eta1**2)*g(f, 22.0) + \
            (11.96 * eta1 * np.exp(0.70 * (1 - rt)))   / ((f - 183.310)**2 + 11.14 * eta1**2) + \
            (0.081 * eta1 * np.exp(6.44 * (1 - rt)))   / ((f - 321.226)**2 +  6.29 * eta1**2) + \
            (3.660 * eta1 * np.exp(1.60 * (1 - rt)))   / ((f - 325.153)**2 +  9.22 * eta1**2) + \
            (25.37 * eta1 * np.exp(1.09 * (1 - rt)))   / ((f - 380.000)**2 ) + \
            (17.40 * eta1 * np.exp(1.46 * (1 - rt)))   / ((f - 448.000)**2 ) + \
            (844.6 * eta1 * np.exp(0.17 * (1 - rt)))   / ((f - 557.000)**2 ) * g(f, 557.0) + \
            (290.0 * eta1 * np.exp(0.41 * (1 - rt)))   / ((f - 752.000)**2 ) * g(f, 752.0) + \
            (8.3328e4 * eta2 * np.exp(0.99 * (1 - rt)))/ ((f - 1780.00)**2 ) * g(f, 1780.0)) * \
                f**2 * rt**2.5 * rho * 1e-4
        return gammaw
    ...
    def gamma0_approx(self, f, P, T):
        ...
        def phi(rp, rt, a, b, c, d) : return np.power(rp, a) * np.power(rt, b) *\
                        np.exp(c * (1 - rp) + d*(1 - rt))
                        

        # Dry air attenuation (gamma0) computation as in Section 1 of Annex 2 of [1]             
    ...
    def gaseous_attenuation_exact(self, f, p, rho, T):
        ...
        tmp = load_data(dataset_dir + '676//v10_lines_oxygen.txt', skip_header=1)
        f_ox = tmp[:,0]; a1 = tmp[:,1]; a2 = tmp[:,2]; a3 = tmp[:,3]; 
        a4 = tmp[:,4]; a5 = tmp[:,5]; a6 = tmp[:,6]
        
        tmp = load_data(dataset_dir + '676//v10_lines_water_vapour.txt', skip_header=1)
        f_wv = tmp[:,0]; b1 = tmp[:,1]; b2 = tmp[:,2]; b3 = tmp[:,3]; 
        b4 = tmp[:,4]; b5 = tmp[:,5]; b6 = tmp[:,6]
        
        D_f_ox = a3 * 1e-4 * (p * np.power(theta*np.ones_like(a4),(0.8 - a4)) + 1.1*e*theta)
        D_f_wv = b3 * 1e-4 * (p * np.power(theta*np.ones_like(b4), b4) + np.power(b5*e*theta, b6))
                
        D_f_ox = np.sqrt(D_f_ox**2 + 2.25e-6)
        D_f_wv = 0.535*D_f_wv + np.sqrt(0.217 * D_f_wv**2 + 2.1316e-12*f_wv**2/theta)
        
        delta_ox = (a5 + a6 * theta) * 1e-4 * (p + e) * theta**0.8
        
        F_i_ox = f/f_ox * ((D_f_ox - delta_ox * (f_ox - f))/((f_ox - f)**2 + D_f_ox**2) + 
                         (D_f_ox - delta_ox * (f_ox + f))/((f_ox + f)**2 + D_f_ox**2))
    ...
    def gaseous_attenuation_approximation(self, f, el, rho, P, T):
        ...
        """
        if f > 350:
            raise ValueError('Frequency must be introduced in GHz and the maximum range is 350 GHz')
            
        # Water vapour attenuation (gammaw) computation as in Section 1 of Annex 2 of [1]
    ...
    def slant_inclined_path_coefficients(self, f, p):
        ...
        t2 = (0.14 * np.exp(2.21 * rp)) /((f - 118.75)**2 + 0.031 * np.exp(2.2 * rp))
    ...
    @classmethod                   
    def gaseous_attenuation_terrestrial_path(self, r, f, el, rho, P, T, mode='approx'):
        """
        """            
        if mode == 'approx':
            gamma0, gammaw = self.gaseous_attenuation_approximation(f, el, rho, P, T)
    ...
    @classmethod    
    def gaseous_attenuation_slant_path(self, f, el, rho, T, P, mode='approx'):
        """
        """            
        if mode == 'approx' and 5 < el and el < 90:
            gamma0, gammaw = self.gaseous_attenuation_approximation(f,el, rho, P, T)
            ...
            Agas = 0
            for t, press, r, delta, n_r in zip(T_n, press_n, r_n, delta_h, n_ratio):
                a = - r * np.cos(b) + 0.5 * np.sqrt( 4 * r**2 * np.cos(b)**2 + 8 * r * delta + 4*delta**2)
    ...
    @classmethod
    def gaseous_attenuation_inclined_path(self, f, el, rho, P, T, h1, h2, mode='approx'):
        """
        """
        if h1 > 10 or h2 > 10:
            raise ValueError('Both the transmitter and the receiver must be at' +
            'altitude of less than 10 km above the sea level. Current altitude' +
            ' Tx: %.2f km, Rx: %.2f km' % (h1, h2))
        ...
        if 5 < el and el < 90:                
            ...
            def eq_33(h_num, h_den, el, x):
                return np.sqrt(Re + h_num) * F(x) * np.exp(-h_num/h_den) / np.cos(np.deg2rad(el))
...
class _ITU676_9():
    ...
    def gaseous_attenuation_exact(self, f, p, rho, T):
        ...
        tmp = load_data(dataset_dir + '676//v9_lines_oxygen.txt', skip_header=1)
        f_ox = tmp[:,0]; a1 = tmp[:,1]; a2 = tmp[:,2]; a3 = tmp[:,3]; 
        a4 = tmp[:,4]; a5 = tmp[:,5]; a6 = tmp[:,6]
        
        tmp = load_data(dataset_dir + '676//v9_lines_water_vapour.txt', skip_header=1)
        f_wv = tmp[:,0]; b1 = tmp[:,1]; b2 = tmp[:,2]; b3 = tmp[:,3]; 
        b4 = tmp[:,4]; b5 = tmp[:,5]; b6 = tmp[:,6]
...
def gaseous_attenuation_terrestrial_path(r, f, el, rho, P, T, mode):
    ...
    * 'exact': an estimate of gaseous attenuation computed by summation of
    individual absorption lines that is valid for the frequency range 1-1,000 GHz
...
def gaseous_attenuation_inclined_path(f, el, rho, P, T, h1, h2, mode='approx'):
    ...
    * 'exact': an estimate of gaseous attenuation computed by summation of
    ...
    val = __model.gaseous_attenuation_inclined_path(f, el, rho, P, T, h1, h2, mode=mode)
    return prepare_output_array(val, type_output) * u.dB
...
def zenit_water_vapour_attenuation(lat, lon, p, f, V_t=None, alt=None):
    ...
    p : number
        Percentage of the time the zenit water vapour attenuation value is exceeded.         
    f : number or Quantity
        Frequency (GHz)
    V_t : number or Quantity, optional
        Integrated water vapour content along the path (kg/m2 or mm). 
        If not provided this value is estimated using Recommendation ITU-R P.836.
        Default value None
    ...
    lon = np.mod(lon, 360)
    f = prepare_quantity(f, u.GHz, 'Frequency')
    V_t = prepare_quantity(V_t, u.kg / u.m**2, 'Integrated water vapour content along the path')
    val = __model.zenit_water_vapour_attenuation(lat, lon, p, f, V_t=V_t, alt=alt)
    return prepare_output_array(val, type_output) * u.dB
