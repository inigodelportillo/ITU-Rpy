# -*- coding: utf-8 -*-
import numpy as np
from itur1144 import bicubic_2D_interpolator
...
class __ITU1510():
    ...
    * P.1510-0 (02/01) (Current version)
    ...
    def mean_temperature(self, lat, lon):
        # Abstract method to compute the surface mean temperature
        return self.instance.mean_temperature(lat, lon)
...
class _ITU1510_0():
    ...
    def temperature(self, lat, lon):
        if not self._temperature:
            ...
            lons = load_data(dataset_dir + '1510/v0_Lon.txt.txt')
    ...
    def surface_mean_temperature(self, lat_d, lon_d):
        """
        Method to compute the annual mean surface temperature (°C) at 2 m 
...
def surface_mean_temperature(lat, lon):
    """
    A method to estimate the annual mean surface temperature (°C) at 2 m 
    above the surface of the Earth
    ...
    return prepare_output_array(val, type_output) * u.Celsius
