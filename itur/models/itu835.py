...
class _ITU835_6:
    ...
    def standard_pressure(h, T_0=None, P_0=None):
        ...
        P = np.where(
            h_p <= 11,
            1013.25 * np.power((288.15 / (288.15 - 6.5 * h_p)), (-34.1632 / 6.5)),
            np.where(
                np.logical_and(11 < h_p, h_p <= 20),
                ...
                np.where(
                    ...
                    np.where(
                        np.logical_and(32 < h_p, h_p <= 47),
                        8.680422
                        * (228.65 / (228.65 + 2.8 * (h_p - 32))) ** (34.1632 / 2.8),
                        np.where(
                            np.logical_and(47 < h_p, h_p <= 51),
                ...
                np.where(
                    ...
                    np.where(
                        ...
                        np.where(
                            ...
                            np.where(
                                np.logical_and((80 <= h), (h <= 100)), 175, np.nan
                            ),
                        ),
                    ),
                ),
    ...
    def mid_latitude_water_vapour_summer(h):
        ...
        return np.where(
            np.logical_and((0 <= h), (h <= 15)),
            14.3542 * np.exp(-0.4174 * h - 0.02290 * h**2 + 0.001007 * h**3),
            0,
        )
    ...
    def mid_latitude_temperature_winter(h):
        ...
        return np.where(
            ...
            np.where(
                np.logical_and((10 <= h), (h < 33)),
                218,
                np.where(
                    np.logical_and((33 <= h), (h < 47)),
                    218 + (h - 33) * 3.3571,
                    np.where(
                        np.logical_and((47 <= h), (h < 53)),
                        265,
                        np.where(
                            ...
                            265 - (h - 53) * 2.0370,
                            np.where(
                                np.logical_and((80 <= h), (h <= 100)), 210, np.nan
                            ),
                        ),
                    ),
                ),
    ...
    def mid_latitude_water_vapour_winter(h):
        ...
        return np.where(
            np.logical_and(0 <= h, h <= 10),
            3.4742 * np.exp(-0.2697 * h - 0.03604 * h**2 + 0.0004489 * h**3),
            0,
        )
    ...
    def high_latitude_temperature_summer(h):
        ...
        return np.where(
            ...
            np.where(
                np.logical_and((10 <= h), (h < 23)),
                225,
                np.where(
                    np.logical_and((23 <= h), (h < 48)),
                    225 * np.exp((h - 23) * 0.008317),
                    np.where(
                        np.logical_and((48 <= h), (h < 53)),
                        277,
                        np.where(
                            np.logical_and((53 <= h), (h < 79)),
                            277 - (h - 53) * 4.0769,
                            np.where(
                                np.logical_and((79 <= h), (h <= 100)), 171, np.nan
                            ),
                        ),
                    ),
                ),
    ...
    def high_latitude_water_vapour_summer(h):
        ...
        return np.where(
            np.logical_and((0 <= h), (h <= 15)),
            8.988 * np.exp(-0.3614 * h - 0.005402 * h**2 - 0.001955 * h**3),
            0,
        )
    ...
    def high_latitude_temperature_winter(h):
        ...
        return np.where(
            np.logical_and((0 <= h), (h < 8.5)),
            257.4345 + 2.3474 * h - 1.5479 * h**2 + 0.08473 * h**3,
            np.where(
                np.logical_and((8.5 <= h), (h < 30)),
                217.5,
                np.where(
                    np.logical_and((30 <= h), (h < 50)),
                    217.5 + (h - 30) * 2.125,
                    np.where(
                        np.logical_and((50 <= h), (h < 54)),
                        260,
                        np.where(
                            np.logical_and((54 <= h), (h <= 100)),
                            260 - (h - 54) * 1.667,
                            np.nan,
                        ),
                    ),
    ...
    def high_latitude_water_vapour_winter(h):
        ...
        return np.where(
            np.logical_and((0 <= h), (h <= 10)),
            1.2319 * np.exp(0.07481 * h - 0.0981 * h**2 + 0.00281 * h**3),
            0,
        )
    ...
    def temperature(self, lat, h, season="summer"):
        ...
        if season == "summer":
            return np.where(
                np.abs(lat) < 22,
                self.low_latitude_temperature(h),
                np.where(
                    np.abs(lat) < 45,
                    self.mid_latitude_temperature_summer(h),
            ...
            return np.where(
                ...
                np.where(
                    np.abs(lat) < 45,
                    self.mid_latitude_temperature_winter(h),
                    self.high_latitude_temperature_winter(h),
                ),
            )
        else:
            raise ValueError(
                "The value for argument 'season' is not correct."
    ...
    def pressure(self, lat, h, season="summer"):
        ...
        if season == "summer":
            return np.where(
                np.abs(lat) < 22,
                self.low_latitude_pressure(h),
                np.where(
                    np.abs(lat) < 45,
                    self.mid_latitude_pressure_summer(h),
            ...
            return np.where(
                ...
                np.where(
                    np.abs(lat) < 45,
                    self.mid_latitude_pressure_winter(h),
    ...
    def water_vapour_density(self, lat, h, season="summer"):
        ...
        if season == "summer":
            return np.where(
                np.abs(lat) < 22,
                self.low_latitude_water_vapour(h),
                np.where(
                    np.abs(lat) < 45,
                    self.mid_latitude_water_vapour_summer(h),
            ...
            return np.where(
                ...
                np.where(
                    np.abs(lat) < 45,
                    self.mid_latitude_water_vapour_winter(h),
