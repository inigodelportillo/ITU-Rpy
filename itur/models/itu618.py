...
from itur.models.itu453 import (
...
def __CDF_bivariate_normal__(alpha_x, alpha_y, rho):
    ...
    def CDF_bivariate_normal_fcn(x, y, rho):
        return np.exp(-(x**2 - 2 * rho * x * y + y**2) / (2.0 * (1 - rho**2)))

    def CDF_bivariate_normal_int(alpha, y, rho):
        return scipy.integrate.quad(
            CDF_bivariate_normal_fcn, alpha, np.inf, args=(y, rho)
        )[0]
    ...
    return (
        ...
        * scipy.integrate.quad(
            lambda y: CDF_bivariate_normal_int(alpha_x, y, rho), alpha_y, np.inf
        )[0]
    )
...
class _ITU618:
    ...
    def rain_attenuation(
        self, lat, lon, f, el, hs=None, p=0.01, R001=None, tau=45, Ls=None
    ):
        fcn = np.vectorize(
            self.instance.rain_attenuation,
            excluded=[0, 1, 3, 4, 6],
            otypes=[np.ndarray],
        )
    ...
    def rain_attenuation_probability(self, lat, lon, el, hs, Ls, P0=None):
        fcn = np.vectorize(
            self.instance.rain_attenuation_probability,
            excluded=[0, 1, 2],
            otypes=[np.ndarray],
        )
    ...
    def rain_cross_polarization_discrimination(self, Ap, f, el, p, tau):
        fcn = np.vectorize(self.instance.rain_cross_polarization_discrimination)
        return fcn(Ap, f, el, p, tau)

    def scintillation_attenuation(self, lat, lon, f, el, p, D, eta, T, H, P, hL):
        fcn = np.vectorize(
            self.instance.scintillation_attenuation,
            excluded=[0, 1, 3, 7, 8, 9],
            otypes=[np.ndarray],
        )
        return np.array(fcn(lat, lon, f, el, p, D, eta, T, H, P, hL).tolist())

    def scintillation_attenuation_sigma(self, lat, lon, f, el, p, D, eta, T, H, P, hL):
        fcn = np.vectorize(
            self.instance.scintillation_attenuation_sigma,
            excluded=[0, 1, 3, 7, 8, 9],
            otypes=[np.ndarray],
        )
        return np.array(fcn(lat, lon, f, el, p, D, eta, T, H, P, hL).tolist())
    ...
    def site_diversity_rain_outage_probability(
        self, lat1, lon1, a1, el1, lat2, lon2, a2, el2, f, tau=45, hs1=None, hs2=None
    ):
        fcn = np.vectorize(self.instance.site_diversity_rain_outage_probability)
        return np.array(
            fcn(lat1, lon1, a1, el1, lat2, lon2, a2, el2, f, tau, hs1, hs2).tolist()
        )
...
class _ITU618_13:
    ...
    def rain_attenuation(
        ...
        if Ls is None:
            Ls = np.where(
                ...
                * (hr - hs)
                / (
                    ((np.sin(np.deg2rad(el))) ** 2 + 2 * (hr - hs) / Re) ** 0.5
        ...
        r001 = 1.0 / (
            1 + 0.78 * np.sqrt(Lg * gammar / f) - 0.38 * (1 - np.exp(-2 * Lg))
        )
        ...
        v001 = 1.0 / (
            1
            + np.sqrt(np.sin(np.deg2rad(el)))
            * (
                31 * (1 - np.exp(-(el / (1 + xi)))) * np.sqrt(Lr * gammar) / f**2
                - 0.45
            )
        )
    ...
    def rain_attenuation_probability(self, lat, lon, el, hs=None, Ls=None, P0=None):
        ...
        # rate data
        if P0 is None:
            P0 = rainfall_probability(lat, lon).to(u.dimensionless_unscaled).value
    ...
    def fit_rain_attenuation_to_lognormal(self, lat, lon, f, el, hs, P_k, tau):
        ...
        m_lna, sigma_lna = np.linalg.lstsq(
            np.vstack([np.ones(len(Q)), Q]).T, lnA, rcond=None
        )[0]
    ...
    @classmethod
    def site_diversity_rain_outage_probability(
        self, lat1, lon1, a1, lat2, lon2, a2, f, el1, el2, tau=45, hs1=None, hs2=None
    ):
        ...
        P_1 = rainfall_probability(lat1, lon1).to(u.dimensionless_unscaled).value

        P_2 = rainfall_probability(lat2, lon2).to(u.dimensionless_unscaled).value
    ...
    def rain_cross_polarization_discrimination(self, Ap, f, el, p, tau):
        ...
        if scale_to_orig_f:
            ...
            XPD_p = XPD_p - 20 * np.log10(
                f_orig
                * np.sqrt(1 - 0.484 * (1 + np.cos(np.deg2rad(4 * tau))))
                / (f * np.sqrt(1 - 0.484 * (1 + np.cos(np.deg2rad(4 * tau)))))
            )
        return XPD_p

    @classmethod
    def scintillation_attenuation_sigma(
        self, lat, lon, f, el, p, D, eta=0.5, T=None, H=None, P=None, hL=1000
    ):
        ...
        L = (
            2
            * hL
            / (np.sqrt(np.sin(np.deg2rad(el)) ** 2 + 2.35e-4) + np.sin(np.deg2rad(el)))
        )  # Eq. 44   [m]
        ...
        g = np.where(
            ...
            0,
            np.sqrt(
                3.86 * (x**2 + 1) ** (11.0 / 12) * np.sin(11.0 / 6 * np.arctan2(1, x))
                - 7.08 * x ** (5.0 / 6)
            ),
        )  # Eq. 46    [-]
        ...
        sigma = sigma_ref * f ** (7.0 / 12) * g / np.sin(np.deg2rad(el)) ** 1.2
        return sigma
    ...
    def scintillation_attenuation(
        ...
        # applicable period and propagation path:
        sigma = self.scintillation_attenuation_sigma(
            lat, lon, f, el, p, D, eta, T, H, P, hL
        )
...
def rain_attenuation(lat, lon, f, el, hs=None, p=0.01, R001=None, tau=45, Ls=None):
    """
...
def scintillation_attenuation(
    lat, lon, f, el, p, D, eta=0.5, T=None, H=None, P=None, hL=1000
):
    """
