...
class __ITU453__:
    ...
    def __init__(self, version=13):
        if version == 13:
            ...
            raise ValueError(
                "Version {0} is not implemented for the ITU-R P.453 model.".format(
                    version
                )
            )
    ...
    def map_wet_term_radio_refractivity(self, lat, lon, p=50):
        fcn = np.vectorize(
            self.instance.map_wet_term_radio_refractivity,
            excluded=[0, 1],
            otypes=[np.ndarray],
        )
    ...
    def DN65(self, lat, lon, p):
        fcn = np.vectorize(self.instance.DN65, excluded=[0, 1], otypes=[np.ndarray])
        return np.array(fcn(lat, lon, p).tolist())

    def DN1(self, lat, lon, p):
        fcn = np.vectorize(self.instance.DN1, excluded=[0, 1], otypes=[np.ndarray])
        return np.array(fcn(lat, lon, p).tolist())
...
class _ITU453_13_:
    ...
    def DN65(self, lat, lon, p):
        ...
        return self._DN65[float(p)](np.array([lat.ravel(), lon.ravel()]).T).reshape(
            lat.shape
        )
    ...
    def DN1(self, lat, lon, p):
        ...
        return self._DN1[float(p)](np.array([lat.ravel(), lon.ravel()]).T).reshape(
            lat.shape
        )
    ...
    def N_wet(self, lat, lon, p):
        ...
        return self._N_wet[float(p)](np.array([lat.ravel(), lon.ravel()]).T).reshape(
            lat.shape
        )
    ...
    def map_wet_term_radio_refractivity(self, lat, lon, p):
        ...
        lats = np.array(
            [90 - R * 0.75, 90 - (R + 1) * 0.75, 90 - R * 0.75, 90 - (R + 1) * 0.75]
        )

        lons = np.array([C * 0.75, C * 0.75, (C + 1) * 0.75, (C + 1) * 0.75]) - 180
        ...
        if not pExact:
            rho = N_wet_b + (N_wet_a - N_wet_b) * (np.log(p) - np.log(p_below)) / (
