...
class __ITU838__:
    ...
    * P.838-3 (03/05) (Current version)
    ...
    def rain_specific_attenuation_coefficients(self, f, el, tau):
        ...
        fcn = np.vectorize(
            self.instance.rain_specific_attenuation_coefficients,
            excluded=[1],
            otypes=[np.ndarray],
        )
...
class _ITU838_3_:
    ...
    def rain_specific_attenuation_coefficients(f, el, tau):
        ...
        KH = np.power(
            10,
            sum([curve_fcn(f, kh["aj"][j], kh["bj"][j], kh["cj"][j]) for j in range(4)])
            + kh["mk"] * np.log10(f)
            + kh["ck"],
        )
        KV = np.power(
            10,
            sum([curve_fcn(f, kv["aj"][j], kv["bj"][j], kv["cj"][j]) for j in range(4)])
            + kv["mk"] * np.log10(f)
            + kv["ck"],
        )
        ...
        alphaH = (
            sum(
                [
                    curve_fcn(f, alphah["aj"][j], alphah["bj"][j], alphah["cj"][j])
                    for j in range(5)
                ]
            )
        ...
        alphaV = (
            sum(
                [
                    curve_fcn(f, alphav["aj"][j], alphav["bj"][j], alphav["cj"][j])
                    for j in range(5)
                ]
            )
        ...
        k = (
            KH
            + KV
            + (KH - KV) * np.cos(np.deg2rad(el)) ** 2 * np.cos(np.deg2rad(2 * tau))
        ) / 2.0
...
class _ITU838_2_:
    ...
    def rain_specific_attenuation_coefficients(f, el, tau):
        ...
        alphah = {
            "aj": [0.5564, 0.2237, -0.1961, -0.02219],
        ...
        alphav = {
            "aj": [0.5463, 0.2158, -0.1693, -0.01895],
        ...
        KH = np.power(
            10,
            sum([curve_fcn(f, kh["aj"][j], kh["bj"][j], kh["cj"][j]) for j in range(3)])
            + kh["mk"] * np.log10(f)
            + kh["ck"],
        )
        KV = np.power(
            10,
            sum([curve_fcn(f, kv["aj"][j], kv["bj"][j], kv["cj"][j]) for j in range(3)])
            + kv["mk"] * np.log10(f)
            + kv["ck"],
        )
...
class _ITU838_0_:
    ...
    def rain_specific_attenuation_coefficients(*args, **kwargs):
        return _ITU838_1_.rain_specific_attenuation_coefficients(*args, **kwargs)
