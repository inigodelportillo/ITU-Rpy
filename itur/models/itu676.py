...
def __gamma0_exact__(self, f, p, rho, T):
    ...
    D_f_ox = self.a3 * 1e-4 * (p * (theta ** (0.8 - self.a4)) + 1.1 * e * theta)
...
def __gammaw_exact__(self, f, p, rho, T):
    ...
    D_f_wv = self.b3 * 1e-4 * (p * theta**self.b4 + self.b5 * e * theta**self.b6)

    D_f_wv = 0.535 * D_f_wv + np.sqrt(
        0.217 * D_f_wv**2 + 2.1316e-12 * f_wv**2 / theta
    )
...
class __ITU676__:
    ...
    def slant_inclined_path_equivalent_height(self, f, p, rho, T):
        fcn = np.vectorize(
            self.instance.slant_inclined_path_equivalent_height,
            excluded=[0],
            otypes=[np.ndarray],
        )
    ...
    def zenit_water_vapour_attenuation(self, lat, lon, p, f, V_t=None, h=None):
        ...
        # slant path
        fcn = np.vectorize(
            self.instance.zenit_water_vapour_attenuation,
            excluded=[0, 1, 4, 5],
            otypes=[np.ndarray],
        )
...
class _ITU676_12_:
    ...
    tmp = load_data(
        os.path.join(dataset_dir, "676/v12_lines_oxygen.txt"), skip_header=1
    )
    f_ox = tmp[:, 0]
    a1 = tmp[:, 1]
    a2 = tmp[:, 2]
    ...
    tmp = load_data(
        os.path.join(dataset_dir, "676//v12_lines_water_vapour.txt"), skip_header=1
    )
    f_wv = tmp[:, 0]
    b1 = tmp[:, 1]
    b2 = tmp[:, 2]
    ...
    @classmethod
    def gamma_exact(self, f, p, rho, T):
        return self.gamma0_exact(f, p, rho, T) + self.gammaw_exact(f, p, rho, T)
    ...
    def slant_inclined_path_equivalent_height(self, f, P, rho, T):
        ...
        t2 = sum(
            [
                (ci * np.exp(2.12 * rp)) / ((f - fi) ** 2 + 0.025 * np.exp(2.2 * rp))
    ...
    def gaseous_attenuation_slant_path(
        self, f, el, rho, P, T, V_t=None, h=None, mode="approx"
    ):
        """ """
        if mode == "approx":
            gamma0, gammaw = self.gaseous_attenuation_approximation(f, el, rho, P, T)
            ...
            if V_t is not None and h is not None:
                Aw = self.zenit_water_vapour_attenuation(None, None, None, f, V_t, h)
            else:
                Aw = gammaw * hw
            ...
            h_n = 0.0001 * (
                (np.exp(np.arange(0, 922) / 100.0) - 1.0) / (np.exp(1.0 / 100.0) - 1.0)
            )  # Eq. 15
    ...
    def gaseous_attenuation_inclined_path(
        ...
        if 5 < el and el < 90:
            ...
            A = gamma0 * np.sqrt(h0) * (
                eq_33(h1, h0, el1, xi(el1, h1)) - eq_33(h2, h0, el2, xi(el2, h2))
            ) + gammaw * np.sqrt(hw) * (
                eq_33(h1, hw, el1, xi_p(el1, h1)) - eq_33(h2, hw, el2, xi_p(el2, h2))
            )
            return A
    ...
    def zenit_water_vapour_attenuation(self, lat, lon, p, f, V_t=None, h=None):
        ...
        a = (
            0.2048 * np.exp(-(((f - 22.43) / 3.097) ** 2))
            + 0.2326 * np.exp(-(((f - 183.5) / 4.096) ** 2))
            + 0.2073 * np.exp(-(((f - 325) / 3.651) ** 2))
            - 0.1113
        )
...
class _ITU676_11_:
    ...
    tmp = load_data(
        os.path.join(dataset_dir, "676/v11_lines_oxygen.txt"), skip_header=1
    )
    f_ox = tmp[:, 0]
    a1 = tmp[:, 1]
    a2 = tmp[:, 2]
    ...
    tmp = load_data(
        os.path.join(dataset_dir, "676//v11_lines_water_vapour.txt"), skip_header=1
    )
    f_wv = tmp[:, 0]
    b1 = tmp[:, 1]
    b2 = tmp[:, 2]
    ...
    def slant_inclined_path_equivalent_height(self, f, p, rho=None, T=None):
        ...
        hw = 1.66 * (
            1
            + (1.39 * sigmaw) / ((f - 22.235) ** 2 + 2.56 * sigmaw)
...
class _ITU676_10_:
    ...
    tmp = load_data(
        os.path.join(dataset_dir, "676/v10_lines_oxygen.txt"), skip_header=1
    )
    f_ox = tmp[:, 0]
    a1 = tmp[:, 1]
    a2 = tmp[:, 2]
    ...
    tmp = load_data(
        os.path.join(dataset_dir, "676//v10_lines_water_vapour.txt"), skip_header=1
    )
    f_wv = tmp[:, 0]
    b1 = tmp[:, 1]
    b2 = tmp[:, 2]
    ...
    def gamma0_approx(self, f, P, rho, T):
        ...
        def phi(rp, rt, a, b, c, d):
            return (
                np.power(rp, a) * np.power(rt, b) * np.exp(c * (1 - rp) + d * (1 - rt))
            )
        ...
        gamma0 = np.where(
            ...
            fcn_le_54(),
            np.where(
                np.logical_and(54 < f, f <= 60),
                fcn_le_60(),
                np.where(
                    np.logical_and(60 < f, f <= 62),
                    fcn_le_62(),
                    np.where(
                        np.logical_and(62 < f, f <= 66),
                        fcn_le_66(),
                        np.where(
                            np.logical_and(66 < f, f <= 120), fcn_le_120(), fcn_rest()
                        ),
                    ),
                ),
            ),
...
class _ITU676_9_:
    ...
    tmp = load_data(
        os.path.join(dataset_dir, "676//v9_lines_oxygen.txt"), skip_header=1
    )
    f_ox = tmp[:, 0]
    a1 = tmp[:, 1]
    a2 = tmp[:, 2]
    ...
    tmp = load_data(
        os.path.join(dataset_dir, "676//v9_lines_water_vapour.txt"), skip_header=1
    )
    f_wv = tmp[:, 0]
    b1 = tmp[:, 1]
    b2 = tmp[:, 2]
    ...
    def slant_inclined_path_equivalent_height(*args, **kwargs):
        return _ITU676_10_.slant_inclined_path_equivalent_height(*args, **kwargs)

    @staticmethod
    def gaseous_attenuation_terrestrial_path(*args, **kwargs):
        return _ITU676_10_.gaseous_attenuation_terrestrial_path(*args, **kwargs)
...
def gaseous_attenuation_slant_path(f, el, rho, P, T, V_t=None, h=None, mode="approx"):
    """
    ...
    P = prepare_quantity(P, u.hPa, "Atospheric pressure")
    T = prepare_quantity(T, u.K, "Temperature")
    V_t = prepare_quantity(V_t, u.kg / u.m**2, "Integrated water vapour content")
    h = prepare_quantity(h, u.km, "Altitude")
    val = __model.gaseous_attenuation_slant_path(f, el, rho, P, T, V_t, h, mode)
    return prepare_output_array(val, type_output) * u.dB
...
def zenit_water_vapour_attenuation(lat, lon, p, f, V_t=None, h=None):
    ...
    V_t = prepare_quantity(
        V_t, u.kg / u.m**2, "Integrated water vapour content along the path"
    )
    h = prepare_quantity(h, u.km, "Altitude")
