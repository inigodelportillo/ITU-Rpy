...
from itur453 import water_vapour_pressure, wet_term_radio_refractivity,\
                    map_wet_term_radio_refractivity
from itur837 import rainfall_rate,rain_percentage_probability
from itur838 import rain_specific_attenuation
from itur839 import rain_height
from itur1511 import topographic_altitude       
from utils import prepare_input_array, prepare_output_array, prepare_quantity,\
...
class _ITU618_12():
    ...
    def rain_attenuation(self, lat, lon, f, el, hs = None, p = 0.01, R001 = None,
        ...
        if p < 0.001 or p > 5:
            raise ValueError('p must be comprised in the range 0.001 % to 5 %')
    ...
    def rain_attenuation_probability(self, lat, lon, el, hs=None, Ls=None, P0=None):
        Re = 8500
        if hs is None:
            hs = topographic_altitude(lat, lon).to(u.km).value
        ...
        # Step 5: Calculate the probability of rain attenuation on the slant path:
    ...
    def fit_rain_attenuation_to_lognormal(self, lat, lon, f, el, hs, P_k):
        ...
        # Step 2: Transform the set of pairs [Pi, Ai] to [Q^{-1}(Pi/P_k), ln(Ai)]
    ...
    def rain_cross_polarization_discrimination(self, Ap, f, el, p, tau):
        ...
        if f < 4 or f > 55:
            raise ValueError('f must be comprised in the range 4 GHz - 55 GHz')        
        
        if el > 60:
            raise ValueError('el must be comprised in the range 0 deg - 60 deg') 
    ...
    def scintillation_attenuation(self, lat, lon, f, el, p, D, eta=0.5, T=None,
        ...
        if T is not None and H is not None and P is not None:
            e_s = water_vapour_pressure(T, P, H)
        
            # Step 2: Compute the wet term of the radio refractivity, Nwet, 
            # corresponding to es, t and H as given in Recommendation ITU-R P.453.
            N_wet = wet_term_radio_refractivity(e_s, T)
        else:
        ...
        x = 1.22 * D_eff**2 * f/L
        g = np.where(x >= 7.0, 0,
                np.sqrt(3.86 * (x**2 + 1)**(11/22) * np.sin(11/6*np.arctan2(1, x)) -\
                7.08 * x**(5/6)))                  #Eq. 46    [-]
...
def rain_attenuation(lat, lon, f, el, hs = None, p = 0.01, R001 = None,
    ...
    References
    ----------
    [1] Propagation data and prediction methods required for the design of Earth-space
    telecommunication systems: https://www.itu.int/dms_pubrec/itu-r/rec/p/R-REC-P.618-12-201507-I!!PDF-E.pdf
    """
    global __model
    ...
    tau = prepare_quantity(tau, u.mm/u.hr, 'Polarization tilt angle')
...
def rain_attenuation_probability(lat, lon, el, hs=None, Ls=None, P0=None):
    ...
    lon = prepare_input_array(lon)
    el = prepare_quantity(el, u.deg, 'Elevation angle')
    Ls = prepare_quantity(Ls, u.km, 'Heigh above mean sea level of the earth station')
...
def site_diversity_rain_outage_probability(self, lat1, lon1, a1, el1, lat2,
    ...
    lat1 = prepare_quantity(lat1, u.deg, 'Latgitude in ground station 2')
    lon1 = prepare_quantity(lon1, u.deg, 'Longitude in ground station 2')
    ...
    hs1 = prepare_quantity(hs1, u.deg, 'Altitude over the sea level for ground station 1')
    hs2 = prepare_quantity(hs1, u.deg, 'Altitude over the sea level for ground station 2')
    
    val = __model.site_diversity_rain_outage_probability(lat1, lon1, a1, lat2,
                                               lon2, a2, f, el1, el2, hs1=hs1,
                                               hs2=hs2)
...
def scintillation_attenuation(lat, lon, f, el, p, D, eta=0.5, T=None,
    ...
    f = prepare_quantity(f, u.GHz, 'Frequency')
    el = prepare_quantity(el, u.deg, 'Elevation angle')
    p = prepare_quantity(p, u.km, 'Heigh above mean sea level of the earth station')
    D = prepare_quantity(D, u.mm/u.hr, 'Point rainfall rate')
    eta = prepare_quantity(eta, u.mm/u.hr, 'Polarization tilt angle')
    T = prepare_quantity(T, u.km, 'Slant path length')
    H = prepare_quantity(H, u.km, 'Slant path length')
    P = prepare_quantity(P, u.km, 'Slant path length')
    hL = prepare_quantity(hL, u.km, 'Slant path length')

    val = __model.scintillation_attenuation(lat, lon, f, el, p, D, eta=eta, T=T,
                                  H=H, P=P, hL=hL)
